"""
Strict Evaluation: S1 Spatial XGBoost vs Locked Tuned Champion.

Evaluates the existing experimental S1 models against the locked tuned champion models
...
- Saves results to:
    ml/results/spatial_s1_vs_champion.json
    ml/results/spatial_s1_vs_champion.csv
...
OUTPUT_JSON = RESULTS_DIR / "spatial_s1_vs_champion.json"
OUTPUT_CSV = RESULTS_DIR / "spatial_s1_vs_champion.csv"
...
S1_MODEL_FILES = {
    "6h": MODELS_DIR / "xgb_spatial_pm25_6h_experimental.joblib",
    "24h": MODELS_DIR / "xgb_spatial_pm25_24h_experimental.joblib",
    "72h": MODELS_DIR / "xgb_spatial_pm25_72h_experimental.joblib",
}
...
def compute_relative_improvement(champ_val: float, s1_val: float) -> float:
    """Positive -> S1 better (lower error). Negative -> S1 worse."""
    if champ_val == 0:
        return 0.0
    return float((champ_val - s1_val) / champ_val * 100.0)
...
def main():
    logger.info("=" * 78)
    logger.info("STRICT COMPARISON: S1 SPATIAL XGBOOST vs LOCKED TUNED CHAMPION")
    logger.info("=" * 78)
    ...
    for h in HORIZONS:
        ...
        s_path = S1_MODEL_FILES[h]
        if not c_path.exists():
            raise FileNotFoundError(f"Champion model missing: {c_path}")
        if not s_path.exists():
            raise FileNotFoundError(f"S1 experimental model missing: {s_path}")
        logger.info(f"  [{h}] Champion: {c_path.name} | S1: {s_path.name}")
    ...
    # 4. Compute S1 spatial features
    logger.info("Computing S1 spatial features via ml.spatial_features.add_spatial_features...")
    ...
    for h in HORIZONS:
        ...
        X_s1 = sub_df[s_feats].values

        # Load models
        champ_model = joblib.load(CHAMPION_MODEL_FILES[h])
        s1_model = joblib.load(S1_MODEL_FILES[h])

        # Predict
        pred_champ = champ_model.predict(X_champ)
        pred_s1 = s1_model.predict(X_s1)

        # Integrity Check F: Verify exact count and origin matching
        assert len(y_true) == len(pred_champ) == len(pred_s1), f"Length mismatch for horizon {h}"
        
        # Overall metrics
        metrics_champ = compute_standard_metrics(y_true, pred_champ)
        metrics_s1 = compute_standard_metrics(y_true, pred_s1)

        delta_mae_pct = compute_relative_improvement(metrics_champ["MAE"], metrics_s1["MAE"])
        delta_rmse_pct = compute_relative_improvement(metrics_champ["RMSE"], metrics_s1["RMSE"])

        sub_df["pred_champ"] = pred_champ
        sub_df["pred_s1"] = pred_s1
        sub_df["err_champ"] = pred_champ - y_true
        sub_df["err_s1"] = pred_s1 - y_true
        sub_df["abs_err_champ"] = np.abs(sub_df["err_champ"])
        sub_df["abs_err_s1"] = np.abs(sub_df["err_s1"])
        ...
        for stn in EXPECTED_STATIONS:
            ...
            if stn_count > 0:
                ...
                csv_rows.append({
                    ...
                    "s1_mae": round(stn_m_s["MAE"], 3),
                    "delta_mae_pct": round(stn_delta_mae, 2),
                    "champion_rmse": round(stn_m_c["RMSE"], 3),
                    "s1_rmse": round(stn_m_s["RMSE"], 3),
                    "champion_bias": round(stn_m_c["Bias"], 3),
                    "s1_bias": round(stn_m_s["Bias"], 3),
                })
        ...
        for r_name, r_min, r_max in REGIME_BINS:
            ...
            if r_count > 0:
                ...
                r_m_s = compute_standard_metrics(y_true[r_mask], pred_s1[r_mask])
                ...
                csv_rows.append({
                    ...
                    "s1_mae": round(r_m_s["MAE"], 3),
                    "delta_mae_pct": round(r_delta_mae, 2),
                    "champion_rmse": round(r_m_c["RMSE"], 3),
                    "s1_rmse": round(r_m_s["RMSE"], 3),
                    "champion_bias": round(r_m_c["Bias"], 3),
                    "s1_bias": round(r_m_s["Bias"], 3),
                })
        ...
        for thresh in HIGH_POLLUTION_THRESHOLDS:
            ...
            if hp_count > 0:
                ...
                hp_m_s = compute_standard_metrics(y_true[hp_mask], pred_s1[hp_mask])
                ...
                csv_rows.append({
                    ...
                    "s1_mae": round(hp_m_s["MAE"], 3),
                    "delta_mae_pct": round(hp_delta_mae, 2),
                    "champion_rmse": round(hp_m_c["RMSE"], 3),
                    "s1_rmse": round(hp_m_s["RMSE"], 3),
                    "champion_bias": round(hp_m_c["Bias"], 3),
                    "s1_bias": round(hp_m_s["Bias"], 3),
                })
    ...
    # -----------------------------------------------------------------------
    # Rules:
    # PASS: S1 improves MAE by >= 2% on at least two horizons and does not materially degrade remaining horizon (>= -1.0%).
    ...
    if n_pass >= 2 and all_not_degraded:
        verdict = "PASS"
        verdict_desc = "S1 improves MAE by >= 2% on at least two horizons without material degradation on the remaining horizon."
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 90)
    print("STRICT EVALUATION REPORT: S1 SPATIAL XGBOOST vs LOCKED TUNED CHAMPION")
    print(f"Test Period: {TEST_START} to {TEST_END}  (Untouched Out-of-Time Holdout)")
    print("=" * 90)
    print(f"{'Horizon':<8} {'Obs':>7} | {'Champ MAE':>10} {'S1 MAE':>9} {'Delta MAE%':>10} | {'Champ RMSE':>11} {'S1 RMSE':>10} {'Delta RMSE%':>11} | {'Champ R2':>9} {'S1 R2':>8}")
    print("-" * 90)
    for h in HORIZONS:
    ...
    print("-" * 90)
    print(f"{'Horizon':<8} {'Regime':<8} {'Obs':>6} | {'Champ MAE':>10} {'S1 MAE':>9} {'Delta MAE%':>10} | {'Champ Bias':>11} {'S1 Bias':>10}")
    print("-" * 90)
    for h in HORIZONS:
        ...
        for r_name in [">=150", ">=250"]:
            ...
            print(f"{h:<8} {r_name:<8} {sub['count']:>6,} | {sub['champion']['MAE']:>10.3f} {sub['s1']['MAE']:>9.3f} {dm:>+9.2f}% | {sub['champion']['Bias']:>11.3f} {sub['s1']['Bias']:>10.3f}")
    print("=" * 90)
