...
DATA_FILE = resolve_path("data/processed/fusion/delhi_forecasting_satellite.csv")
HORIZON_METADATA_FILE = resolve_path("models/horizon_features.json")
BEST_PARAMS_FILE = resolve_path("ml/results/satellite/s3_best_params.json")
...
print("=" * 70)
print("FINAL S3 SATELLITE XGBOOST TRAINING")
print("=" * 70)
...
for horizon, target in HORIZONS.items():
    ...
    print("\n" + "=" * 70)
    print(f"TRAINING S3 — {horizon}")
    print("=" * 70)

    canonical_features = metadata["horizon_features"][horizon]

    feature_cols = canonical_features + SATELLITE_FEATURES
    ...
    print(f"S3 features:       {len(feature_cols)}")
    ...
    model_path = MODEL_DIR / f"xgb_satellite_pm25_{horizon}_tuned.joblib"
...
summary_path = REPORT_DIR / "s3_final_test_results.csv"

results_df.to_csv(summary_path, index=False)

print("\n" + "=" * 70)
print("FINAL S3 TRAINING COMPLETE")
print("=" * 70)
