...
# ============================================================
# S8-B2 — Event-Level Lead-Time Analysis
# ============================================================
...
# We only evaluate 6h and 24h, Gate B (prediction + episode)
HORIZONS = {
    6: {
...
def run_horizon_analysis(df, horizon, cfg):
    ...
    FP = int(((test_pred_bool == True) & (y_test == 0)).sum())
    TP = int(((test_pred_bool == True) & (y_test == 1)).sum())
    precision = TP / (TP + FP) if (TP + FP) > 0 else 0.0

    print(f"Row-level Test FP (False Alarms): {FP}")
    print(f"Row-level Test TP (Detections): {TP}")

    # 7. Event-level lead time
    df_test = df.loc[test_mask].loc[test_usable].copy()
    df_test["predicted_bool"] = test_pred_bool

    events_df = get_events(df, VALID_END, TEST_END)
    print(f"Total true severe events (crossings) in test period: {len(events_df)}")

    df_test_indexed = df_test.set_index(["station_id", "timestamp"]).sort_index()
    ...
    for _, row in events_df.iterrows():
        ...
        alarms = preds[preds["predicted_bool"] == True]
        if len(alarms) > 0:
            earliest_alarm_time = alarms.index.min()
            lt = (t_onset - earliest_alarm_time).total_seconds() / 3600.0
            event_lead_times.append(lt)
        else:
            event_lead_times.append(None)

    # 8. Calculate Bucket Metrics
    total_events = len(event_lead_times)

    for b in buckets:
        detected_lts = [lt for lt in event_lead_times if lt is not None and lt >= b]
        n_det = len(detected_lts)
        n_miss = total_events - n_det
        det_rate = n_det / total_events if total_events > 0 else 0.0
        ...
        if n_det > 0:
            ...
            med_lt = mean_lt = p25_lt = min_lt = max_lt = fa_per_ev = 0.0

        print(f"\nBucket: >={b}h before onset")
        print(f"  Actual severe events: {total_events}")
        print(f"  Events detected     : {n_det}")
        print(f"  Events missed       : {n_miss}")
        ...
        print(f"  False-alarm ratio   : {1.0 - precision:.4f}")
        print(f"  Precision           : {precision:.4f}")
        print(f"  FA per detected ev  : {fa_per_ev:.2f}")
