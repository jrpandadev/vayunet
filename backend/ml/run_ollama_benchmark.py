...
# Strip additionalProperties from the JSON Schema so Ollama structured output handles it smoothly
...
def run_ollama_model(model_name: str, bundle: Dict[str, Any]) -> Dict[str, Any]:
...
Generate the investigation report based strictly on the evidence provided above."""
    ...
    # Strip markdown codeblocks if Ollama accidentally adds them around the JSON
...
def main():
    models = ["qwen3.5:4b", "gemma3:4b", "phi4-mini:3.8b"]
    ...
    base_out_dir = backend_root / "reports/investigation/ollama_model_benchmark"
    ...
    for model in models:
        ...
        for bundle_name in bundle_names:
            ...
            output_file = model_out_dir / f"INVESTIGATION_{bundle_name}"
            ...
            try:
                result = run_ollama_model(model, bundle_data)
