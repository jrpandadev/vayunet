...
WEB2API_URL = "http://127.0.0.1:8081/v1/chat/completions"
...
def run_web2api_model(bundle: Dict[str, Any]) -> Dict[str, Any]:
...
Generate the investigation report based strictly on the evidence provided above."""
    ...
    payload = {
        ...
        "response_format": {
            "type": "json_schema",
            "json_schema": {
                "name": "PollutionEventInvestigationReport",
    ...
    resp = requests.post(WEB2API_URL, json=payload, headers=headers, timeout=300)
    latency = round(time.time() - start_time, 2)

    if resp.status_code != 200:
        raise Exception(f"Web2API error {resp.status_code}: {resp.text}")
    ...
    if message_content.startswith("```"):
        if "```json" in message_content:
            message_content = message_content.split("```json")[1]
        else:
            message_content = message_content.split("```")[1]
        message_content = message_content.rsplit("```", 1)[0]
...
def main():
    ...
    base_out_dir = backend_root / "reports/investigation/gemini_web2api_pilot"
    base_out_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n{'='*50}\nEvaluating Model: gemini-3.6-flash via Web2API\n{'='*50}")
    ...
    for bundle_name in bundle_names:
        ...
        try:
            result = run_web2api_model(bundle_data)
    ...
    print("\n\nBenchmark complete. Please review reports.")
